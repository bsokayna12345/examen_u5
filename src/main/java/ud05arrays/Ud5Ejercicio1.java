...
public class Ud5Ejercicio1 {
	 ...
	 * como parametro.
	 * 
	 * @param datosBolas datos de las bolas en array bidimensional de numero de
	...
	public static Virus[] crearArrayVirus(Object[][] datosVirus) {
		Virus[] virus = new Virus[datosVirus.length];
		for (int i = 0; i < virus.length; i++) {
			virus[i] = new Virus((String) datosVirus[i][0], (Integer) datosVirus[i][1], (Float) datosVirus[i][2]);
	...
	public static void imprimeVirus(Virus[] array, String titulo, boolean pares) {
		...
		for (int i = in; i < array.length; i = i + 2) {
	...
	public static Virus[] ordena(final Virus[] array) {
		Virus[] arrayOrder = array.clone();
		for (int i = 0; i < arrayOrder.length; i++) {
			for (int j = i + 1; j < arrayOrder.length; j++) {
				if (arrayOrder[i].compareTo(arrayOrder[j]) > 0) {
					Helper.swap(arrayOrder, i, j);
				}
	...
	public static Virus[] eliminaVirus(Virus v, final Virus[] array) {
		Virus[] arrayCopiado = array.clone();
		// int j=0;
		if (v != null) {

			for (int i = 0; i < arrayCopiado.length; i++) {
				if (arrayCopiado[i] != null && v.equals(arrayCopiado[i])) {
					arrayCopiado[i] = null;

					for (int j = i; j < arrayCopiado.length - 1; j++) {
						arrayCopiado[j] = arrayCopiado[j + 1];
						Helper.swap(arrayCopiado, j, j + 1);
					}

				}
//		Virus []  arrayNuevo = new Virus[arrayCopiado.length];
//		for (int i = j; i < arrayCopiado.length-1; i++) {
//			
//			if(arrayCopiado[i]==null) {
//		arrayNuevo[i]		
//		}
//			
//		
...
//		
	...
	public static int[] encuentraPeligrosos(Virus v, Virus[] array) {
		...
		int contador = 0;
		int k = 0;
		Float u = v.getR0() * v.getLetalidad();
		for (int i = 0; i < array.length; i++) {
			Float l = array[i].getR0() * array[i].getLetalidad();
			if (l >= u) {
				peligrosos[k] = i;
				contador++;
		...
		int[] indix = null;
		if (contador >= 1) {
			indix = new int[peligrosos.length- contador];

			for (int i = 0; i <peligrosos.length; i++) {
				indix[i] = peligrosos[i];
